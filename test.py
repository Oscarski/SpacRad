...
while (now_time < start_time + timedelta(minutes=mission_time)):
    ...
    try:
        ...
        # zdjecie i jasnosc
        image_name = "spacerad_{}.jpg".format(str(photo_counter).zfill(4))
        camera.capture(image_name)
        image = Image.open("spacerad_{}.jpg".format(str(photo_counter).zfill(4)))
        jasnosc = calculate_brightness(image)

        # zapisywanie informacji do pliku log
        info_log = "\tZdjecie {}:".format(photo_counter), jasnosc
